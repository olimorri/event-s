...
exports.deleteEvent = async (req, res) => {
  try {
    ...
    const deleteFromHost = await Users.findByIdAndUpdate(
      event.owner,
      { $pull: { eventList: event._id } },
      { new: true },
    );
...
exports.attendEvent = async (req, res) => {
  try {
    const { id } = req.params;
    const { _id } = req.user;
    const addToEvent = await Events.findByIdAndUpdate(
      id,
...
exports.unattendEvent = async (req, res) => {
  try {
    const { id } = req.params;
    const { _id } = req.user;
    const deleteFromEvent = await Events.findByIdAndUpdate(
      id,
      { $pull: { list: _id }, $inc: { attendees: -1 } },
      { new: true },
    );
    const deleteFromUser = await Users.findByIdAndUpdate(
      _id,
      { $pull: { eventList: id } },
      { new: true },
    );
